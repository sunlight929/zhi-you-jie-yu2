...
# 知情同意书（仅测试模式 + 首次进入显示；完整版下立即返回 True）
render_consent_banner()
...
# ============================================================
# 🤖 AI 心理健康助手 fragment 定义
# ============================================================
# - 完整版（评委 / 答辩演示）：渲染到左侧侧边栏（保持原行为）
# - 测试版（手机扫码用户）：渲染到主页面"综合判读"之后（无需翻最底下）
# fragment 必须在调用前定义，所以放在 if is_student 之前。

@st.fragment
def render_llm_chat_content():
    """AI 心理健康助手对话内容（仅内容，不含容器切换）。"""
...
<div style="margin-top:8px; font-size:12px; color:#666;">{phq9_sev['interpretation']}</div>
        ...
        with col_score2:
            ...
            # 测试模式下软化标签（"中度风险"→"中等风险信号"），完整版保留
            if is_test_mode():
                ml_advice = {**ml_advice,
                             "level": soften_severity_label(ml_advice["level"])}
            st.markdown(f"""
<div class="score-card">
...
{verdict}
        ...
        # 新评估开始 → 重置聊天历史 + 反馈状态（允许再次反馈）
        st.session_state["llm_messages"] = []
        st.session_state.pop("feedback_submitted", None)

        # ====================================================
        # 【测试模式】AI 助手 + 反馈表（紧贴"综合判读"下方）
        # ====================================================
        if is_test_mode():
            st.markdown("---")
            render_llm_chat_content()
            # 反馈表直接跟在 AI 之后，不被"个性化建议"打断
            render_feedback_form()

        # ---------- 三模型详情 + 贡献图（仅完整版显示，测试版隐藏以缩短页面） ----------
        if not is_test_mode():
            st.markdown("### 🔬 模型细节")
...
- **综合风险：{result['final_proba']:.3f}**
        ...
        # ---------- 匿名数据收集（完整版 & 测试版都写入，mode 字段区分） ----------
        save_anonymous_response({
            "group": "student",
            "scale_name": "PHQ-9",
            "scale_score": phq9_total,
            "scale_severity": phq9_sev["level"],
            "ml_proba": round(float(result["ml_proba"]), 4),
            "final_proba": round(float(result["final_proba"]), 4),
            "age": age,
            "gender": gender,
            "grade": grade,
            "major": major,
            "top_factor_1": top3[0] if len(top3) > 0 else "",
            "top_factor_2": top3[1] if len(top3) > 1 else "",
            "top_factor_3": top3[2] if len(top3) > 2 else "",
            "sleep_hours": sleep,
            "smoking": int(smoking),
...
# ============================================================
# AI 助手渲染位置：
#   - 完整版：左侧侧边栏（独立于评估提交，未提交时显示提示）
#   - 测试版：已在 if submitted 块内部"综合判读"之后渲染（移到主页面）
# ============================================================
if not is_test_mode():
    with st.sidebar:
        st.markdown("---")
